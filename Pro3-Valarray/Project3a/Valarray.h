...
namespace epl {
	...
	template <typename> struct SRank;
	...
	template <typename T> struct isComplex { static constexpr int bool_value = 0; };
	template <typename T> struct isComplex<std::complex<T>> { static constexpr int bool_value = 1; };
	...
	public:
		...
		wrapper<UnaryProxy<wrapper<T>, std::negate<typename T::value_type>>> operator-(void) {
			using neg_op = typename std::negate<typename T::value_type>;
			return wrapper<UnaryProxy<wrapper<T>, neg_op>>(UnaryProxy<wrapper<T>, neg_op>(*this, neg_op{}));
		}

	};

	template <typename T1, typename T2, typename OP = std::plus<typename choose_Vtype<wrapper<T1>, wrapper<T2>>::type>>
	wrapper<BinaryProxy<T1, T2, OP>> operator+(const wrapper<T1>& lhs, const wrapper<T2>& rhs) {
	...
	wrapper<BinaryProxy<T1, Scalar<T2>, OP>> operator+(const wrapper<T1>& lhs, const T2& rhs) {
	...
	wrapper<BinaryProxy<Scalar<T1>, T2, OP>> operator+(const T1& lhs, const wrapper<T2>& rhs) {
